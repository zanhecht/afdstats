...
FOOTER = '<footer>Bugs, suggestions, questions?  Contact the maintainers at <a href="http://en.wikipedia.org/wiki/User_talk:Enterprisey">User talk:Enterprisey</a> and <a href="http://en.wikipedia.org/wiki/User_talk:Σ">User talk:Σ</a> • <a href="https://gitlab.wikimedia.org/toolforge-repos/afdstats" title="afdstats on Wikimedia GitLab">Source code</a></footer>'
...
<div style="width:875px;">""")
    try:
        ...
        if len(results) == 0:
            errorout(
                "No AfD's found. This user may not exist.  Note that if the user's username does not appear in the wikitext of their signature, you may need to specify an alternate name."
            )

        print(
            '<p>These statistics were compiled by an automated process, and may contain errors or omissions due to the wide variety of styles with which people cast votes at AfD.  Any result fields which contain "UNDETERMINED" were not able to be parsed, and should be examined manually.</p>'
        )
        ...
        if startdate:
            ...
            print(
                "Total number of unique AfD pages edited by "
                + username
                + " (from "
                + datestr
                + " and earlier): "
            ...
            print(
                "Analyzed the last "
                + str(min(maxsearch, len(results)))
                + " AfD pages edited by this user.<br />"
            )
            ...
            try:
                ...
                if len(dupvotes) < 1:
                    if is_nominator:  # user is nominator
                        ...
                        print(
                            "<li><a href = 'https://en.wikipedia.org/wiki/Wikipedia:"
        ...
        if totalvotes > 0:
            ...
            for i in votetypes:
                print(
                    ...
                    + str(stats[i])
                    + " ("
                    + str(round((100.0 * stats[i]) / totalvotes, 1))
                    + "%)</li>"
                )
            print("</ul>")
            if novotes:
                print(
                    "The remaining "
                    + str(novotes)
                    + " pages had no discernible vote by this user."
                )
            print("<br />")
...
<tbody>\n"""
            ...
            printstr += (
                ...
                + '"><small>Next '
                + str(maxsearch)
                + " AfD's &rarr;</small></a><br /><br />"
            )

            total_votes = sum(matchstats)
            if total_votes > 0:
                print(
                    "Number of AfD's where vote matched result (green cells): {} ({:.1%})<br>".format(
                        matchstats[0], float(matchstats[0]) / total_votes
                    )
                )
                print(
                    "Number of AfD's where vote didn't match result (red cells): {} ({:.1%})<br>".format(
                ...
                if total_votes != matchstats[2]:
                    print(
                        "Without considering \"No Consensus\" results, <b>{:.1%} of AfD's were matches</b> and {:.1%} of AfD's were not.".format(
...
def APIfirsteditor(
    p,
):  # Finds the name of the user who created a particular page, using the API.  Deprecated, using db query instead, see DBfirsteditor()
    try:
        u = urlopen(
            "http://en.wikipedia.org/w/api.php?action=query&prop=revisions&titles=Wikipedia:"
            + urllib.parse.quote(p)
            + "&rvlimit=1&rvprop=timestamp|user&rvdir=newer&format=xml"
        )
        xml = u.read()
        u.close()
        s = re.search(
            '<rev user="(?P<user>.*?)" timestamp="(?P<timestamp>.*?)" />', xml
        )
        user = s.group("user")
        ...
        timestamptext = (
            monthmap[timestamp.group(2)]
            + " "
            + timestamp.group(3).lstrip("0")
            + ", "
            + timestamp.group(1)
        )
...
def errorout(
    ...
    print(
        "<p>ERROR: "
        + errorstr
        + "</p><p>Please <a href='http://afdstats.toolforge.org/'>try again</a>.</p>"
    )
    print(FOOTER)
    print("</div></body>\n</html>")
    sys.exit(0)
